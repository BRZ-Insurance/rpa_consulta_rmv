...
###################################
driver_list = []
index_rpa = 0  #### COMO VAMOS TRABALHAR COM O MÁXIMO DE INSTÂNCIAS POSSÍVEIS, NÃO HÁ NECESSIDADE DE ARMAZENAR EM UMA VARIÁVEL
n = 0   #### SUMIR COM ESSA VARIÁVEL E USAR O LEN(PROCESSING)
...
def init():
    ...
    global index_rpa
    global n
    for index,webdriver in enumerate(driver_list):
        # TODO instanciar aqui TODOS os ChromeDRIVERS
        pass
...
def rmv(request: VIN):
    ...
    if len(processing) < index_rpa:  ## index rpa = 2
        print(f'\nPARÂMETROS INICIAIS:\ndriver_list:{driver_list}\nindex_rpa:{index_rpa}\nn:{n}')
        t = CustomThread(target=bot,args=[request.vin,driver_list[n]])  #### len(PROCESSING)
        n += 1
        if n == index_rpa:
            n = 0

        processing.append(t.start())  ## processing = 2
    ...
    elif len(processing) == index_rpa:  #### len(DRIVERLIST)
        print(f'\nPARÂMETROS INICIAIS:\ndriver_list:{driver_list}\nindex_rpa:{index_rpa}\nn:{n}')
        global fila
        ...
        for i in range(600):
            ...
            time.sleep(2)
            while len(processing) < index_rpa:  ### len(DRIVERLIST)
                req = fila.pop(0)
